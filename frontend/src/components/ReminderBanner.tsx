...
export function ReminderBanner() {
  ...
  // Which pending project (if any) is the status-update modal open for.
  // Owned here so the reminder chips can pop the write form directly
  // — the previous /board?project=X link opened the read-only detail
  // panel and forced the PM to hunt for the update button.
          ...
          {data.pending.slice(0, 6).map((p) => {
            ...
            return (
              <li key={p.project_id}>
                <button
                  type="button"
                  onClick={() => setStatusModalId(p.project_id)}
